package com.nukateam.gunscore.client.data.handler;

import com.nukateam.gunscore.Config;
import com.nukateam.gunscore.client.input.GunButtonBindings;
import com.nukateam.gunscore.client.screen.WorkbenchScreen;
...
import com.nukateam.gunscore.common.network.message.C2SMessageAttachments;
import com.nukateam.gunscore.common.network.message.C2SMessageUnload;
...
public class ControllerHandler {
    ...
    public void updateAvailableActions(GatherActionsEvent event) {
        ...
        if (player != null) {
            ItemStack heldItem = player.getMainHandItem();
            if (heldItem.getItem() instanceof GunItem) {
                event.getActions().put(GunButtonBindings.AIM, new Action(new TranslatableComponent("gunscore.action.aim"), Action.Side.RIGHT));
                event.getActions().put(GunButtonBindings.SHOOT, new Action(new TranslatableComponent("gunscore.action.shoot"), Action.Side.RIGHT));

                GunItem gunItem = (GunItem) heldItem.getItem();
                Gun modifiedGun = gunItem.getModifiedGun(heldItem);
                CompoundTag tag = heldItem.getTag();
                if (tag != null && tag.getInt("AmmoCount") < GunEnchantmentHelper.getAmmoCapacity(heldItem, modifiedGun)) {
                    event.getActions().put(GunButtonBindings.RELOAD, new Action(new TranslatableComponent("gunscore.action.reload"), Action.Side.LEFT));
                }

                Scope scope = Gun.getScope(heldItem);
                if (scope != null && scope.isStable() && AimingHandler.get().isAiming()) {
                    event.getActions().put(GunButtonBindings.STEADY_AIM, new Action(new TranslatableComponent("gunscore.action.steady_aim"), Action.Side.RIGHT));
                }
            }
        }
    }

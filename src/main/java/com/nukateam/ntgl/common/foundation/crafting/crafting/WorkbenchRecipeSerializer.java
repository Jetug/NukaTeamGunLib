    ...
    public MapCodec<WorkbenchRecipe> codec() {
        return RecordCodecBuilder.mapCodec(instance ->
                instance.group(
                        ...
                        // Поле materials (список ингредиентов)
                        WorkbenchIngredient.CODEC.listOf().fieldOf("materials").forGetter(WorkbenchRecipe::getMaterials)
                ).apply(instance, (result, materials) ->
                        new WorkbenchRecipe(null, result, ImmutableList.copyOf(materials))
                )
        );
    }

    @Override
    public StreamCodec<FriendlyByteBuf, WorkbenchRecipe> streamCodec() {

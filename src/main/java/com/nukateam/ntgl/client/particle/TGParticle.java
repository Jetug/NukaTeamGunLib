...
import com.nukateam.ntgl.client.render.TGRenderHelper;
...
//	public double posZ;
	...
	public TGParticle(Level worldIn, double xCoordIn, double yCoordIn, double zCoordIn, double xSpeedIn,
		...
		private void init() {
			...
			this.xo+=(systemVelocityFactor*particleSystem.xo());
			this.yo+=(systemVelocityFactor*particleSystem.yo());
			this.zo+=(systemVelocityFactor*particleSystem.zo());
...
//			}
    ...
    public void onUpdate()
    {
		...
		if (this.type.particlesStickToSystem) {
			...
			Vec3d motion = new Vec3d (this.xo, this.yo, this.zo);
		...
		this.yo = velY;
		this.zo = velZ;
		this.yo -= type.gravity; //(0.05d * (double) type.gravity * (double) this.ticksExisted);
		//this.moveEntity(this.xo, this.yo, this.zo);
		//System.out.printf("Velocity=(%.2f / %.2f / %.2f)\n",this.velX, this.velY, this.velZ);
		//System.out.printf("Motion=(%.2f / %.2f / %.2f)\n",this.xo, this.yo, this.zo);
		this.setPosition(this.posX+this.xo, this.posY+this.yo, this.posZ+this.zo);
...
//		}
		...
		if (this.type.groundAligned) {
			...
			//<TEST>
//			Vec3d angles = RenderItemBase.getTransformAngles((LivingEntity)entityIn, EnumHandSide.RIGHT, partialTickTime);

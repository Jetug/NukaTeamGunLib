...
import com.nukateam.ntgl.client.render.hud.cache.HudCache;
...
public class AmmoHud implements IGuiOverlay {
    ...
    protected static final Map<InteractionHand, HudCache> cache = Map.of(
            InteractionHand.MAIN_HAND, new HudCache(InteractionHand.MAIN_HAND),
            InteractionHand.OFF_HAND, new HudCache(InteractionHand.OFF_HAND)
    );

    protected final Minecraft minecraft = Minecraft.getInstance();
    private Colors colors = DEFAULT_COLORS;
    public static final IGuiOverlay AMMO_HUD = new AmmoHud();
    ...
    public void render(ForgeGui gui, GuiGraphics graphics, float partialTick, int width, int height) {
        ...
        cache.forEach((hand, cache) -> {
            ...
            if (heldItem.getItem() instanceof IThrowable && shouldRender(hand, player)) {
                if ((System.currentTimeMillis() - cache.checkAmmoTimestamp) > 200) {
...
//                }
    ...
    protected void renderAmmoCounter(GuiGraphics graphics, HudCache handCache, ItemStack stack, int x, int y) {
//        if(!GunModifierHelper.shouldRenderHud(new GunData(stack, minecraft.player))) return;
    ...
    protected void renderCurrentAmmo(GuiGraphics graphics, HudCache handCache,
                                     int x, int y,
                                     PoseStack poseStack,
    ...
    protected void renderInventoryAmmo(GuiGraphics graphics, HudCache handCache, int x, int y,
    ...
    protected void renderThrowModeIcon(GuiGraphics graphics, HudCache handCache, int width, int height,
                                      String currentAmmoCountText) {
    }

    protected void renderAmmoTypeIcon(GuiGraphics graphics, HudCache handCache, int width, int height, String currentAmmoCountText) {
    ...
    protected int getIconX(HudCache handCache, double textWidth) {
    ...
    protected void updateCache(HudCache handCache, LocalPlayer player, ItemStack stack) {
